...
#[allow(unused)]
trait ArtifactVersion: fmt::Debug {
    fn version(&self) -> &str;
}
...
fn compare_all_items(first: &[Item], second: &[Item]) -> Ordering {
    ...
    for index in 0..cmp::max(first_length, second_length) {
        ...
        let result = match left.as_ref() {
            ...
            Some(&Str(s)) => compare_str_with(s, right),
            Some(&Minus) => {
                match right {
                    Some(&Integer(_)) => Ordering::Less, // 1-1 < 1.0.x
                    Some(&Str(_)) => Ordering::Greater,  // 1-1 > 1-sp
                    Some(&Minus) => {
                        compare_all_items(&first[(index + 1)..], &second[(index + 1)..])
                    }
    ...
    for (index, c) in chars.iter().enumerate() {
        if *c == '.' {
            if index == start_index {
                items.push(Item::Integer(BigInt::from(0)));
            } else {
...
fn to_integer_item(value: &str) -> Item {
    // println!("value: {}", value);
    // 1. 处理前导零（例如 "000123" -> "123"）
    let trimmed = value.trim_start_matches('0');
    let trimmed = if trimmed.is_empty() { "0" } else { trimmed };

    // 2. 解析为 BigInt，不会 panic
    let big_int = BigInt::from_str(trimmed).expect("Invalid number string");
    Item::Integer(big_int)
}
...
fn normalize_sublist(items: &mut Vec<Item>, start_index: usize, end_index: usize) {
    for index in (start_index..end_index).rev() {
        ...
        let is_null = match &items[index] {
            ...
            Item::Integer(i) => i.is_zero(),
            Item::Str(s) => comparable_str_qualifier(s) == RELEASE_VERSION_INDEX,
        };
...
pub fn new_artifact_version(version: &'_ str) -> Maven3ArtifactVersion<'_> {
...

    {
        ...
        assert_eq!(
            ...
            "canonical( {} ) = {} -> canonical: {}",
            version, canonical, parsed_canonical
        );
    }
...
mod tests {
    ...
    const VERSIONS_QUALIFIER: [&'static str; 22] = [
        "1-alpha2snapshot",
        "1-alpha2",
        "1-alpha-123",
        "1-beta-2",
    ...
    const VERSIONS_NUMBER: [&'static str; 25] = [
    ...
    fn check_all_versions_order(versions: &[&'static str]) {
        let c: Vec<Maven3ArtifactVersion> = versions
            .iter()
            .map(|version| Maven3ArtifactVersion::new(version))
            .collect();
