...
#define APK_BLOB_PTR_LEN(p, l) ((apk_blob_t){ .ptr = (char *)(p), .len = (int)(l) })
...
static void apk_blob_spn(apk_blob_t b, int (*ctype)(int), apk_blob_t *out, apk_blob_t *rest) {
    int i = 0;
    while (i < b.len && ctype((unsigned char)b.ptr[i])) i++;
...
static int apk_ctype_version_suffix(int c) {
    return (c >= 'a' && c <= 'z');
}
...
static void token_next(struct token_state *t, apk_blob_t *b)
{
    ...
    case '-':
        ...
        if (!apk_blob_pull_blob_match(b, APK_BLOB_PTR_LEN("-r", 2))) goto invalid;
